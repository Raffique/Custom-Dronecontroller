...
int lxp, lyp, rxp, ryp; 
int threshold;
int incremment = threshold/128;
double stick = 0;
int minpos = -32768, maxpos = 32767, minpwm = 0, maxpwm = 255;
int value;
int secondData = 0;
...
/////////////////////////////////////////////////////////////////////////
  ...
  if(income == true){
    ...
    */
    if(received_data.RY > 0 ){
      if(micros() - lastTime > map(received_data.RY, 0, 32767, 200000, 50)){
      
        stick += received_data.RY;
      ...
      if(micros() - lastTime > map(received_data.RY, 0, -32768, 200000, 50)){
    ...
    //analogVal(received_data.RY); Serial.print("RY: "); Serial.print(secondData); Serial.print("  PosValue: "); Serial.println(received_data.RY);
    income = false;
    reset_data();
  }
...
void analogVal(int data){
  ...
  value = map(secondData, minpos, maxpos, minpwm, maxpwm);

  
  /*
  if(micros() - lastime > joysticklimit){
    stick += value;
    lastime = micros();
  }
  */
